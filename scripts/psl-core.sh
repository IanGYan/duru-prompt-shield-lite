...
RULES_DIR="${PSL_RULES_DIR:-$(cd "$(dirname "$0")/../rules" && pwd)}"
LOG_PATH="${PSL_LOG_PATH:-/Users/durubot/.openclaw/workspace/memory/security-log.jsonl}"
RL_STATE_PATH="${PSL_RL_STATE_PATH:-/Users/durubot/.openclaw/workspace/memory/psl-rate-limit.json}"
...
python3 - "$TYPE" "$MODE" "$RULES_DIR" "$LOG_PATH" "$INPUT" "$ACTOR_ID" "$RL_STATE_PATH" "$RL_MAX_REQ" "$RL_WINDOW_SEC" "$RL_ACTION" <<'PY'
...
def rate_limit_check(path:str, actor:str, now_ts:int, max_req:int, window_sec:int):
    ...
    # lightweight GC for stale actors
...
# action context gate (structured risk tags)
if type_ == "action":
    alow=norm.lower()
    gates=[
